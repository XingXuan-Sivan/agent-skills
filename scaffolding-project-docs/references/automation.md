# 自动化生成

> 何时读本文：需要决定项目的索引怎么维护、发布记录怎么产出时。

## 默认自动化，手动兜底

文档体系里有两类内容**天然容易腐烂**，因为它们必须跟着别的东西一起变：

| 内容 | 依赖什么变化 | 手工维护的结局 |
|---|---|---|
| 目录索引表 | 文档的增删改 | 文档改了、索引忘了，然后没有人再相信索引 |
| 版本与发布记录 | 提交历史与版本号 | 第三个版本之后开始说谎 |

所以规则是：**这两类默认由工具生成**；手写只在用户明确要求、或项目正在迁移途中时才允许。

**「让 AI 帮忙手写」不算自动化。** 它仍然是手动维护，只是换了执行者，而且比人更容易忘记。

## 索引：不需要选型

`gen_index.py` 是零依赖的，任何技术栈直接可用，没有选型问题。
标记约定与命令见分片 `assets/automation/index-generation.md`。

## 发布记录：按栈与渠道选分片

读 `assets/automation/` 下的分片，按各自 `applies_when` 挑选。
**三条路是互补关系，不是互斥选项**——先看渠道，再看栈：

| 分片 | 适用 | 成本 |
|---|---|---|
| `changelog-platform.md` | 托管在 GitHub / GitLab，且以平台 Release 分发 | **零安装**，只需配置平台侧分类文件 |
| `changelog-node.md` | Node，且以 npm / pnpm / yarn 工作区组织 | 装一个 devDependency |
| `changelog-generic.md` | 其他任何技术栈，提交信息遵循约定式格式 | 装一个单文件可执行程序 |

优先顺序：**能用平台自带的就用平台自带的**。只有在「使用者从包管理器拿包、需要仓库内
`CHANGELOG.md`」时才需要后两者。

平台 notes 与仓库内 CHANGELOG 的分工、以及如何按分发渠道取舍，见 `changelog-platform.md`。

**三条都不适用时**（例如项目根本不用 Git）：不启用 `docs/releases/`，CHANGELOG 也不强求。
宁可暂时没有发布记录，也不要留一份手工维护、注定过期的。这个兜底很少会被触发——
它的存在是为了明确「没有就是没有」，而不是给手工维护留后门。

## 索引区块总览

`gen_index.py` 维护三类区块，按需使用，不强制都有：

| 区块 | 内容 | 谁来定 |
|---|---|---|
| `gen:docs` | 本目录的文档表 | 完全由 frontmatter 生成 |
| `gen:subdirs` | 直接子目录表 | 完全由目录结构生成 |
| `gen:nav` | 指向文件的导航条目 | **条目由人写**，生成器只按目标是否存在启停 |

`gen:nav` 同时作用于目录 README 的读者导航与根 `AGENTS.md` 的路由表——
两者都是「条目指向某个文件」，也都会因为目标不存在而变成悬空引用。
链接写法（`[文本](路径)`）与行内代码写法（`` `路径` ``）都认。它的启停机制见 `assets/automation/index-generation.md`。

## 文档站

`docs/` 的结构本身就是为文档站准备的源。站点侧边栏应交给文档站自己生成
（例如 VitePress 的 sidebar 自动生成），**不要在站点配置里再抄一份目录清单**——
那又是一处会脱节的手工索引。

## 加一门栈要做什么

1. 在 `assets/automation/` 下新建 `<用途>-<栈>.md`
2. 写 frontmatter：4 个必填字段 + `applies_when`
3. 写清四件事：装什么、怎么初始化、日常命令、与发布流程如何衔接
4. 拷入项目时**去掉栈后缀**，落为 `docs/development/standards/<用途>.md`

不需要改 `SKILL.md`，也不需要注册——运行时枚举目录发现。
